...
class ArduinoController:
    ...
    def read(self):
        """Reads the latest value from the Serial port (0-1023). Returns None if no new valid data."""
        if not self.serial or not self.serial.is_open:
            return None
            
        try:
            if self.serial.in_waiting > 0:
                # Read everything in the buffer to avoid lag
...
class SoundManager:
    ...
    def __init__(self):
        ...
        # This ensures Python looks in the correct folder for audio files
...
class Game:
    ...
    def physics_step(self, dt):
        ...
        # Debug print: uncomment to see if values arrive
        # if arduino_val is not None: print(f"Arduino Val: {arduino_val}")
        
        if arduino_val is not None:
            # Map 0-1023 to Screen Y coordinates
            # Y range is roughly -limit to +limit (e.g. -350 to 350)
            target_y = ((arduino_val / 1023.0) * (limit * 2)) - limit
            
            # Since the potentiometer is likely noisy, let's invert the Y if it feels backwards,
            # or adjust the range slightly.
            # Usually pot: 0 = GND, 1023 = VCC.
            
            # Direct set or smooth move toward target? Direct is snappier for pot.
            # Invert target_y if up/down is reversed: target_y = -target_y
